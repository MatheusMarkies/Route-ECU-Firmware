...
static int lower_index(const float *axis, int len, float x) {
    ...
    if (x >= axis[len-2]) return len-2; // garante i+1 válido
    int lo = 0, hi = len-1;
    while (hi - lo > 1) {
        int mid = (lo + hi) >> 1;
...
// Interpolação 1D
static float interp1d(const float *x_axis, const float *y_vals, int len, float x) {
    if (len < 2) return y_vals[0];
...
static float interpolate_bilinear(const Table2D_t* table, float rpm, float map_kpa) {
...
static float calculate_air_mass_sd(void) {
    ...
    float v_cyl_l = g_ecuConfig.engine_displacement_l / (float)g_ecuConfig.num_cylinders;
    ...
    float air_mass_g = (g_engineData.map_kpa * v_cyl_l * (ve_percent * 0.01f)) / (R_KPAL_PER_GK * charge_temp_k);
...
static float calculate_enrichments_multipliers(void) {
    ...
    float ae = 1.0f;
    ae = interp1d(g_ecuConfig.table_accel_enrichment.x_axis, g_ecuConfig.table_accel_enrichment.values, g_ecuConfig.table_accel_enrichment.len, fabsf(tpsdot));
...
static inline int closed_loop_enabled(void) {
    ...
    if (g_engineData.ect_celsius < g_ecuConfig.clo.min_ect_c) return 0;
    if (g_engineState.rpm < g_ecuConfig.clo.min_rpm || g_engineState.rpm > g_ecuConfig.clo.max_rpm) return 0;
    if (g_engineData.map_kpa < g_ecuConfig.clo.min_map_kpa || g_engineData.map_kpa > g_ecuConfig.clo.max_map_kpa) return 0;
    return 1;
}
...
static void o2_controller_update(float afr_measured, float afr_target, float dt_s) {
    float err = (afr_target > 0.1f) ? ((afr_target - afr_measured) / afr_target) : 0.0f; // erro relativo
    // STFT = PI curto prazo
    g_ecuConfig.clo.stft += (g_o2.kp * err);
    g_ecuConfig.clo.stft = clampf(g_ecuConfig.clo.stft, -g_ecuConfig.clo.stft_limit, g_ecuConfig.clo.stft_limit);
    ...
    if (integ_condition) {
    	...
    	g_ecuConfig.clo.ltft = clampf(g_ecuConfig.clo.ltft, -g_ecuConfig.clo.ltft_limit, g_ecuConfig.clo.ltft_limit);
    } else {
        // anti-windup leve
...
static float injector_deadtime_ms(float vbat) {
    if (g_ecuConfig.table_injector_deadtime.len <= 0) return 1.0f;
    return interp1d(g_ecuConfig.table_injector_deadtime.x_axis, g_ecuConfig.table_injector_deadtime.values, g_ecuConfig.table_injector_deadtime.len, vbat);
}

// ----------------- DFCO -----------------
static int DFCO_active_conditions(void) {
    // Regras típicas: TPS baixo, MAP baixo, RPM acima de limiar e ECT suficientemente alto
    if (g_engineData.tps_percent > g_ecuConfig.dfco.max_tps_percent) return 0;
    if (g_engineData.map_kpa     > g_ecuConfig.dfco.max_map_kpa)     return 0;
    if (g_engineState.rpm        < g_ecuConfig.dfco.min_rpm)         return 0;
    if (g_engineData.ect_celsius < g_ecuConfig.dfco.min_ect_c)       return 0;
    return 1;
}
...
void EngineControl_Init(void) {
    ...
    if (g_engineState.baro_kpa < 10.0f) g_engineState.baro_kpa = g_ecuConfig.baro_ref_kpa;
...
void EngineControl_RunCalculations(void) {
    ...
    uint32_t now = now_us();
    float dt_s = (last_us == 0) ? (1.0f / 1000.0f) : ((now - last_us) / 1000000.0f);
    last_us = now;
    ...
    if (closed_loop_enabled() && g_engineData.lambda > 0.1f) {
        ...
        clo_gain = clampf(clo_gain, g_ecuConfig.clo.min_gain, g_ecuConfig.clo.max_gain);
    ...
    Injector_SchedulePulse(g_engineState.current_cylinder, g_engineState.final_fuel_pw_ms);

    // 11. Ignição (exemplo: buscar avanço e agendar)
    float ign_deg = interpolate_bilinear(&g_ecuConfig.table_ign_advance, g_engineState.rpm, g_engineData.map_kpa);
    ign_deg = clampf(ign_deg, g_ecuConfig.ign_min_deg, g_ecuConfig.ign_max_deg);
    float dwell_ms = 0;
    Ignition_ScheduleSpark(g_engineState.current_cylinder, ign_deg, dwell_ms);
}
...
void EngineControl_CrankEventCallback(void) {
    ...
    if (last_edge_us != 0) {
        ...
        if (dt_us > 0) {
            ...
            g_engineState.rpm = (uint16_t)clampf(60.0f * rev_per_s, 0.0f, (float)g_ecuConfig.rpm_max);
        }
    }
    last_edge_us = t_us;
    ...
    if (g_engineState.engine_angle_deg >= 720.0f) { // 4 tempos
        ...
        g_engineState.current_cylinder = (g_engineState.current_cylinder % g_ecuConfig.num_cylinders) + 1;
    }
